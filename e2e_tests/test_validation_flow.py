...
def test_bulk_does_not_fail_on_internal_error():
    ...
    with patch.object(vr, "validate_email_internal", side_effect=boom):
        payload = {
...
# MX-specific tests
# ---------------------------
def test_mx_undeliverable_returns_200():
    ...
    with patch.object(vs, "validate_email_mx", side_effect=fake_mx):
        response = client.post("/validation/validate-single?email=good@example.com")
        assert response.status_code == 200
        data = response.json()
