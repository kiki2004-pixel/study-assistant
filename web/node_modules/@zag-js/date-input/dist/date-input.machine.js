...
var machine = (0, import_core.createMachine)({
  props({ props }) {
    ...
    const defaultValue = props.defaultValue ? props.defaultValue.map((date) => (0, import_date_utils.constrainValue)(toTargetCalendar(date), props.min, props.max)) : void 0;
    const value = props.value ? props.value.map((date) => (0, import_date_utils.constrainValue)(toTargetCalendar(date), props.min, props.max)) : void 0;
    ...
    const formatter = props.formatter ?? new import_date.DateFormatter(locale, (0, import_segments.getFormatterOptions)(granularity, digitStyle, hourCycle, timeZone));
  ...
  context({ prop, bindable }) {
    ...
    const hc = (0, import_incomplete_date.resolvedHourCycle)(formatter);
    ...
    segments: (0, import_core.memo)(
      ...
      (_deps, { context, prop, computed }) => {
        ...
        return Array.from({ length: computed("groupCount") }, (_, i) => {
          const dv = displayValues[i] ?? new import_incomplete_date.IncompleteDate(placeholderValue.calendar, (0, import_incomplete_date.resolvedHourCycle)(formatter));
  ...
  implementations: {
    ...
    actions: {
      ...
      clearDisplayValues({ context, prop, computed }) {
        const hc = (0, import_incomplete_date.resolvedHourCycle)(prop("formatter"));
        const placeholderValue = context.get("placeholderValue");
        context.set(
          "displayValues",
          Array.from({ length: computed("groupCount") }, () => new import_incomplete_date.IncompleteDate(placeholderValue.calendar, hc))
        );
      },
      ...
      setPreviousActiveSegmentIndex(ctx) {
        ...
        const { allSegments, segments, offset, localIndex } = (0, import_validity.resolveActiveSegment)(ctx);
        ...
        context.set("activeSegmentIndex", (0, import_validity.getGroupOffset)(allSegments, prevGroupIndex) + lastPrevGroupEditableLocalIndex);
      },
      setNextActiveSegmentIndex(ctx) {
        const { context } = ctx;
        const { segments, offset, localIndex } = (0, import_validity.resolveActiveSegment)(ctx);
      ...
      clearSegmentValue(params) {
        ...
        let dv = (0, import_validity.getActiveDisplayValue)(params);
        if (type === "dayPeriod") {
        ...
        if (newValue === "" || newValue === "0") {
          ...
          (0, import_validity.setDisplayValue)(params, index, dv);
      ...
      invokeOnSegmentAdjust(params) {
        ...
        const dv = (0, import_validity.getActiveDisplayValue)(params);
      ...
      setSegmentValue(params) {
        ...
        const index = context.get("activeIndex");
        (0, import_input.updateSegmentValue)(params, segment, input);
      ...
      setPastedValue({ context, event, prop }) {
        try {
          ...
          const constrained = (0, import_date_utils.constrainValue)(parsed, prop("min"), prop("max"));
          const index = context.get("activeIndex");
      ...
      syncDisplayValues({ context, prop, computed }) {
        ...
        context.set(
          "displayValues",
          (0, import_incomplete_date.initDisplayValues)(value?.length ? value : void 0, placeholderValue, hc, computed("groupCount"))
        );
      },
      ...
      syncPlaceholderProp({ prop, context, computed }) {
        ...
        if (propValue) {
          ...
          if (value?.length) {
            const hc = (0, import_incomplete_date.resolvedHourCycle)(prop("formatter"));
            context.set("displayValues", (0, import_incomplete_date.initDisplayValues)(value, propValue, hc, computed("groupCount")));
          }
        }
      },
...
function commitValue(params, index, dv) {
  ...
  const placeholderValue = context.get("placeholderValue");
  const date = (0, import_date_utils.constrainValue)(dv.toValue(placeholderValue), prop("min"), prop("max"));
  const values = Array.from(context.get("value"));
  values[index] = date;
  context.set("value", values);
}
