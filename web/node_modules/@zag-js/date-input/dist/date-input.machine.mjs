...
import { DateFormatter, toCalendar } from "@internationalized/date";
...
import { createFormatFn, getValueAsString, resolveHourCycleProp, resolvePlaceholderValue } from "./utils/formatting.mjs";
import {
  IncompleteDate,
  incompleteDateEqual,
  incompleteDateHash,
  initDisplayValues,
  resolvedHourCycle
} from "./utils/incomplete-date.mjs";
import { updateSegmentValue } from "./utils/input.mjs";
import { defaultTranslations } from "./utils/placeholders.mjs";
import { parse } from "./date-input.parse.mjs";
import { getFormatterOptions, getSegmentLabel, processSegments, resolveAllSegments } from "./utils/segments.mjs";
import {
...
var machine = createMachine({
  props({ props }) {
    ...
    const defaultValue = props.defaultValue ? props.defaultValue.map((date) => constrainValue(toTargetCalendar(date), props.min, props.max)) : void 0;
    const value = props.value ? props.value.map((date) => constrainValue(toTargetCalendar(date), props.min, props.max)) : void 0;
    ...
    const formatter = props.formatter ?? new DateFormatter(locale, getFormatterOptions(granularity, digitStyle, hourCycle, timeZone));
    ...
    segments: memo(
      ...
      (_deps, { context, prop, computed }) => {
        ...
        return Array.from({ length: computed("groupCount") }, (_, i) => {
          ...
          const segmentFormatter = showEra ? new DateFormatter(locale, {
  ...
  implementations: {
    ...
    actions: {
      ...
      syncDisplayValues({ context, prop, computed }) {
        ...
        context.set(
          "displayValues",
          initDisplayValues(value?.length ? value : void 0, placeholderValue, hc, computed("groupCount"))
        );
      },
