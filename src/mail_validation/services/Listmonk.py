...
class Listmonk:
    ...
    def __init__(self, settings: Settings):
        ...
        self.http_client = httpx.Client(
            ...
            headers={"Authorization": f"token {settings.listmonk_user}:{settings.listmonk_pass}"}
        )

    def subscribers(self, page: int, page_size: int = 100) -> ListmonkData:
        result = self.http_client.get(f"/api/subscribers?page={page}&per_page={page_size}&query=subscribers.status = 'enabled'")
    ...
    def subscriber_by_email(self, email: str) -> Subscriber:
        result = self.http_client.get(f"/api/subscribers?query=subscribers.email = '{email}'")
        return ListmonkData(**result.json()).data.results[0]

    def unsubscribe_subscriber(self, email: str):
        result = self.http_client.put(f"/api/subscribers/query/blocklist", json={"query": f"subscribers.email = '{email}'"})
        return result.json()
