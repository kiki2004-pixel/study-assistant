...
async def test_dns_failure_triggers_unsubscribe(db_url):
    ...
    pages = [[
        ListmonkSubscriber(id=101, email="dead-dns@example.com", created_at="2026-01-01T00:00:00Z"),
        ListmonkSubscriber(id=102, email="live-dns@gmail.com", created_at="2026-01-01T00:01:00Z")
    ]]
    ...
    with patch(DNS_MOCK_PATH, side_effect=[
...
async def test_watermark_not_advanced_on_failure(db_url):
    ...
    store.update_successful_run(list_id=1, last_successful_created_at=initial, 
                                last_run_at=initial, processed_count=0, unsubscribed_count=0)
...
def test_ensure_list_on_conflict(db_url):
    ...
    with engine.begin() as conn:
        count = conn.execute(text("SELECT COUNT(*) FROM listmonk_watermark WHERE list_id = 42")).scalar_one()
    assert count == 1
